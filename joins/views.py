...
def get_ref_id(): 
	...
	try:
		id_exists = join.objects.get(id=join_id)
		get_ref_id()
	except:
		return ref_id 
...
def home(request):
	...
	if form.is_valid():
		...
		if created:
			...
			#add friend who referred us to our join model
			if not obj == None:
				new_join_old.friend = obj
			new_join_old.ip_address = get_ip(request)
			new_join_old.save()
		#print all "friends" that joined as a result of user
		#print Join.objects.filter(friend=obj)
		#print obj.referral.all().count()

		#Redirect user after signup completed
